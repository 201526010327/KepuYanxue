...
@WebServlet("/initHref/*")
public class InitHref extends HttpServlet {
...
//                List<ParamObject> pList = (List<ParamObject>) map.get(key);
//                if(url.trim().length()!=0){
//                    for (ParamObject p : pList){
////                        System.out.println("old url: " + p.getUrl());
//                        p.setUrl(changeUrl(p.getUrl(),url));
////                        System.out.println("new url:" + p.getUrl());
//                    }
//                }else {
//                    pList.get(0).setUrl("");
//                }
//            }
    ...
    private Map<String,Object> getFenleiMap() throws Exception{
        ...
        while(resultSet.next()){
            ...
            String url = "q"+resultSet.getString("url");
            ParamObject p = new ParamObject(id,name,url);
            paramList.add(p);
        }
        map.put("quyu",paramList);
        sql = "select * from theme";
        resultSet = dbHelp.executeQuery(sql);
        List<ParamObject> paramList2 = new ArrayList<ParamObject>();
        while(resultSet.next()){
            int id = resultSet.getInt("id");
            String name = resultSet.getString("name");
